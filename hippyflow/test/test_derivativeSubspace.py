...
class TestDerivativeSubspace(unittest.TestCase):
	def setUp(self):
		...
		self.prior = hf.BiLaplacian2D(self.Vh[hl.PARAMETER],gamma = 0.1, delta = 1.0)
	...
	def buildObservable(self,mesh):
		...
		bc = dl.DirichletBC(self.Vh[hl.STATE], u_bdr, u_boundary)
		bc0 = dl.DirichletBC(self.Vh[hl.STATE], u_bdr0, u_boundary)
		
		def pde_varf(u,m,p):
			return ufl.exp(m)*ufl.inner(ufl.grad(u), ufl.grad(p))*ufl.dx - f*p*ufl.dx

		self.pde = hl.PDEVariationalProblem(self.Vh, pde_varf, bc, bc0, is_fwd_linear=True)
		...
		B = hl.assemblePointwiseObservation(self.Vh[hl.STATE], targets)
