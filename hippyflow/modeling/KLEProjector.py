...
class BoundaryRestrictedKLEProjector:
    ...
    """
    def __init__(self, prior, ds, parameters=KLEParameterList()):
        """Constructor for the BoundaryRestrictedKLEConstructor class.

        Args:
            prior: object that implements the prior
        ...
        self.ds = ds
        self.parameters = parameters
        self.Vh = self.prior.Vh

        self.M = self.make_boundary_restricted_mass_matrix(fill_nullspace=False)  # boundary mass matrix
        self.B = self.make_boundary_restricted_mass_matrix(fill_nullspace=True)  # invertible boundary mass matrix

        self.C = hp.Solver2Operator(self.prior.Rsolver)  # covariance operator
        self.KLE_Operator = MassPreconditionedCovarianceOperator(self.C, self.M)  # mass preconditioned covariance

        # solver for the boundary mass matrix
    ...
    def make_boundary_restricted_mass_matrix(self, fill_nullspace=False):
        ...
        Returns:
            boundary_mass: The boundary mass matrix.
        """
        
        # define trial and test functions, assemble the boundary mass matrix
        u_trial = dl.TrialFunction(self.Vh)
        u_test = dl.TestFunction(self.Vh)
        boundary_mass = dl.assemble(u_trial*u_test*self.ds)

        # add identity to fill the nullspace and ensure invertibility, M_boundary = M_boundary + I
        if fill_nullspace:
            diag = dl.Function(self.Vh).vector()
            ...
            diag.apply('')  # ensure that the changes are applied
    ...
    def construct_input_subspace(self):
        """This method computes the KLE subspace.
        NOTE: The subspace is mass-matrix orthogonal.
        ...
        oversampling = self.parameters['oversampling']
        
        # set up multivector to store GEVP solution
        m_dummy = dl.Function(self.Vh).vector()
        Omega = hp.MultiVector(m_dummy, rank + oversampling)
        hp.parRandom.normal(1.0, Omega)

        # solve the GEVP with (boundary) mass-matrix orthogonality
        d_KLE, KLE_decoder = hp.doublePassG(self.KLE_Operator, self.B, self.Bsolver, Omega, rank, s=1)

        # compute the encoder: M * KLE_decoder
        KLE_encoder = hp.MultiVector(m_dummy, rank)
        hp.MatMvMult(self.M, KLE_decoder, KLE_encoder)

        return d_KLE, KLE_decoder, KLE_encoder
