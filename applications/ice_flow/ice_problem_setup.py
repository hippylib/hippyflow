...
from matern_prior_3d import matern_prior_3d
from ice_linear_observable import ice_linear_observable
...
parser.add_argument('-mesh',dest = 'mesh_file',required = False, default = 'meshes/half_dome_chop_22.xdmf',help = 'mesh file',type = str)
...
parser.add_argument('-delta',dest = 'delta',required=False,default = 0.1, help="delta for matern prior",type=float)
...
mesh_name = args.mesh_file.split('meshes/')[-1].split('.xdmf')[0]
...
mesh = dl.Mesh(mesh_constructor_comm)

with dl.XDMFFile(mesh_constructor_comm,args.mesh_file) as fid:
	fid.read(mesh)

observable_kwargs = {'mesh_file':args.mesh_file,'n_obs':args.n_obs,'output_folder':save_states_dir}
observable = ice_linear_observable(mesh,**observable_kwargs)
# Matern Covariance Prior is instantiated here so that each process can sample m.
Vh = observable.problem.Vh
prior = matern_prior_3d(Vh[PARAMETER],gamma = args.gamma, delta = args.delta)
